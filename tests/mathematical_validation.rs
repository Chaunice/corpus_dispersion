...
fn test_mathematical_properties_symmetry() {
    ...
    assert!((analyzer1.get_sd_population().unwrap() - analyzer2.get_sd_population().unwrap()).abs() < 0.001);
    assert!((analyzer1.get_vc_population().unwrap() - analyzer2.get_vc_population().unwrap()).abs() < 0.001);
}
...
fn test_mathematical_properties_monotonicity() {
    ...
    assert!(jd_uniform > jd_skewed, "Uniform distribution should have higher Juilland D than skewed distribution");
}
...
fn test_mathematical_correctness_adjusted_metrics() {
    ...
    assert!((metrics.ft_adjusted_by_pt.unwrap() - expected_ft_by_pt).abs() < 0.001, 
           "FT adjusted by PT calculation incorrect");
    assert!((metrics.ft_adjusted_by_da.unwrap() - expected_ft_by_da).abs() < 0.001, 
...
fn test_mathematical_correctness_extreme_cases() {
    ...
    let sizes = vec![10.0, 10.0, 10.0];
    let total = 30.0;
    let mut analyzer_concentrated = CorpusWordAnalyzer::new(v_concentrated, sizes.clone(), total).unwrap();
    
    assert_eq!(analyzer_concentrated.get_range(), 1);
    assert_eq!(analyzer_concentrated.get_pervasiveness_pt().unwrap(), 1.0/3.0);
...
fn test_mathematical_correctness_all_metrics_consistency() {
    ...
    assert!(metrics.hellinger_dispersion.unwrap() >= 0.0 && metrics.hellinger_dispersion.unwrap() <= 1.0);
